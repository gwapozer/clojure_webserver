(ns pointsensor-dashboard.views.sensorinfo-edit
  (:require
...
function SaveSensor() {var parms = JquerySerialToHashMap($('form').serialize().replace(/\\+/g,'%20'));$.ajax({type: 'GET', url: '/sensorinfo-multi-save/' + encodeURIComponent(parms), success: function(data) {}, beforeSend: function(){ $('.loader').show()},complete: function(){ $('.loader').hide();}}); }\n
  ...
  (do
    ...
    [:form {:id "edit-form"}
     [:center
      [:table {:border "1"}
       [:tr
        [:th {:align "left"} "Sensor Name"]
        [:th {:align "left"} "Wifi Monitor Name"]
        ]
       (map (fn [result] [:tr
                          [:input {:type "hidden" :id (str "id_" (:id result))  :name (str "id_" (:id result)) :value (:id result)}]
                          [:td {:width "70"}  [:input {:style "color: Grey; opacity: 1;" :readonly true :size "20" :maxlength 10 :type "text" :id (str "userid_" (:id result)) :name (str "userid_" (:id result)) :value (:userid result)}]]
                          [:td {:width "70"}  [:input {:size "20" :maxlength 50 :type "text" :id (str "wifimonitorname_" (:id result)) :name (str "wifimonitorname_" (:id result)) :value (:wifimonitorname result)}]]
                          ]
              ) results)
       ]
      ]
...
(defn edit
  ([entity-list]
   (layout/common-layout "Freezer Assignment"
